...
namespace LVS3
{
    ...
    public static class OracleLabelData
    {
        ...
        public static bool InsertInspectionParamsVDEItem(VDEItem vi, int[] coords, int labelid)
        {
            bool retVal = false;
            string sqlText = "";
            OracleConnection conn = OracleDatabase.GetOracleConnection(Defaults.SchemaToUse);
            try
            {
        ...
        public static bool SaveResultData(string lpn, string lin, string acceptedbyuser, string backingnumber, string selecteditems, string operatorinfo, string reasons, string additionalinfo, int labelindex)
        {
            ...
            {
                if (conn1 == null)
                    conn1 = OracleDatabase.GetOracleConnection(Defaults.SchemaToUse);
                if (conn1.State == ConnectionState.Open)
                {
                    ...
                    oCmd = new OracleCommand(Sql, conn1);
                    oCmd.CommandType = CommandType.Text;
                    oCmd.ExecuteNonQuery();
                    retVal = true;
                }
        ...
        public static bool SaveSummaryDataStart(string reel_lpn, string label_item, string usersaving, int stationid, DateTime started_at)
        {//SaveSummaryDataInspectionStart
            ...
            try
            {
                ...
                if (conn.State == ConnectionState.Open)
                {
                    ...
                    string startDate = " TO_DATE('" + started_at.ToString("MM/dd/yyyy HH:mm:00") + "', 'mm/dd/yyyy HH24:MI:SS')";
                    string endDate = " TO_DATE(TO_CHAR(SYSDATE,'DD-MON-YYYY HH24:MI:SS'),'DD-MON-YYYY HH24:MI:SS')";

                    string Sql = "UPDATE " + Defaults.DB_Schema + ".XX_INSPECTION_REPORT_SUMMARY SET ";
                    Sql = Sql + string.Format("REEL_LPN='{0}', LABEL_ITEM ='{1}', OPERATOR_NAME_SETUP='{2}', MACHINE_NUMBER= {3}, TRAIN_LABEL_START= {4}, TRAIN_LABEL_FINISH={5} WHERE REEL_LPN = '{0}' AND LABEL_ITEM = '{1}'", reel_lpn, label_item, usersaving, stationid, startDate, endDate);
                    oCmd = new OracleCommand(Sql, conn);
                    oCmd.CommandType = CommandType.Text;
                    da.UpdateCommand = oCmd;
                    int res = -1;
                    ...
                    if (res <= 0)
                    {
                        ...
                        Sql = Sql + string.Format("VALUES ('{0}', '{1}', '{2}', {3}, {4}, {5})", reel_lpn, label_item, usersaving, stationid, startDate, endDate);
                        oCmd = new OracleCommand(Sql, conn);
                        oCmd.CommandType = CommandType.Text;
                        oCmd.ExecuteNonQuery();
                    }
        ...
        public static bool SummaryDataInspectionStart(string reel_lpn, string label_item, string userstarting, int stationid, bool cancellinginspection)
        {
            OracleCommand oCmd = new OracleCommand();
            OracleDataAdapter da = new OracleDataAdapter(oCmd);
            bool retVal;
            try
            {
                ...
                if (conn.State == ConnectionState.Open)
                {
                    ...
                    if (label_item.Contains("Issue"))
                    {
                        int endOfLIN = label_item.IndexOf("Issue");
                        label_item = label_item.Substring(0, endOfLIN - 1).Trim();
                    }

                    string Sql = "UPDATE " + Defaults.DB_Schema + ".XX_INSPECTION_REPORT_SUMMARY SET ";
                    Sql = Sql + string.Format("INSPECTION_LABEL_START= {0}, OPERATOR_NAME_INSPECTION = '{1}' WHERE REEL_LPN = '{2}' AND LABEL_ITEM = '{3}' AND MACHINE_NUMBER = {4} ", startInspectionDate, userstarting, reel_lpn, label_item, stationid);
                    oCmd = new OracleCommand(Sql, conn);
                    oCmd.CommandType = CommandType.Text;
                    da.UpdateCommand = oCmd;
                    int res = -1;
        ...
        public static bool SummaryDataInspectionFinish(string reel_lpn, string label_item, string usersaving, int lblcount, int missing, int accept, int reject, int acceptop, int rejectop, string opsummary)
        {
            OracleCommand oCmd = new OracleCommand();
            OracleDataAdapter da = new OracleDataAdapter(oCmd);
            bool retVal;
            try
            {
                ...
                if (conn.State == ConnectionState.Open)
                {
                    ...
                    Sql = Sql + string.Format("COUNT_MISSING = {0}, COUNT_ACCEPTED = {1}, COUNT_REJECTED = {2}, COUNT_ACCEPTED_OP = {3}, COUNT_REJECTED_OP = {4}, OPERATOR_REPORT_SUMMARY = '{5}' WHERE REEL_LPN = '{6}' AND LABEL_ITEM = '{7}'", missing, accept, reject, acceptop, rejectop, opsummary, reel_lpn, label_item);
        ...
        public static bool InspectionParamsUpdateOPZone(int id, VDEItem vdi, string tmp_folder)
        {
            bool retVal = false;
            ...
            try
            {
                ...
                if (conn.State == ConnectionState.Open)
                {
                    ...
                    string tmpfilename = Path.Combine(tmp_folder, "fix.shm");
                    ...
                    if (ret == 1)
                    {
                        tmpfilename = Path.Combine(tmp_folder, vdi.VDEItemName + "_OPZONE_VAM.vam");
                        File.Delete(tmpfilename);
        ...
        public static List<VDEItem> InspectionParamsVDEItemsList(int labelid, List<FontSizesSegment> fontsizessegment, LabelType labeltype)
        {
            ...
            {
                OracleConnection conn = OracleDatabase.GetOracleConnection(Defaults.SchemaToUse);
                sSQL = string.Format("SELECT * FROM " + Defaults.DB_Schema + ".XX_VDE_ITEM_PARAMS WHERE LABEL_ID = {0} AND STATION_ID = {1}", labelid, Defaults.StationID);
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        string barcode_placeholders = dr["BARCODE_PLACEHOLDERS"].ToString();
                        string barcode_non_vde_data = dr["BARCODE_NON_VDE_DATA"].ToString();
                        ...
                        {
                            vdi.BarcodePlaceHolders = new List<string>(barcode_placeholders.Split(','));
                            vdi.BarcodeNonVDEData = barcode_non_vde_data;
                        }
        ...
        public static List<OPZoneData> OPZoneData(int id, string basefolder, LabelType labeltype)
        {
                        ...
                        while (odr.Read())
                        {
                            ...
                            if (!odr.IsDBNull(0))
                            {
                                ...
                                using (FileStream fs = new FileStream(tempFile, FileMode.Create, System.IO.FileAccess.Write))
                                {
                                    ...
                                    ms.Close();
                                    // TODO: ReadShapeModel replacement needed
                                    // ozd.FIXTURE_ID was loaded from HALCON shape model file
                                }
                                File.Delete(tempFile);
                            }
                            blob = null;
                            if (!odr.IsDBNull(1))
                            {
                                ...
                                using (FileStream fs = new FileStream(tempFile, FileMode.Create, System.IO.FileAccess.Write))
                                {
                                    ...
                                    fs.Write(bytes, 0, bytes.Length);
                                    ms.Close();
                                    // TODO: ReadVariationModel replacement needed
                                    // ozd.VARIATION_VAM was loaded from HALCON variation model file
                                }
                                File.Delete(tempFile);
                            }
            ...
            catch (Exception ex)
            {
                ...
                OracleDatabase.notifyError(err, "Data Access", true);
            }
            return retVal;
        }
        ...
        public static string GetVDEContrastAsString(int stationid, string labelitem)
        {
            ...
            int label_id = LabelID(stationid, labelitem);
            DataSet dsFilters = OracleDatabase.GetFilters();

            OracleConnection conn = OracleDatabase.GetOracleConnection(Defaults.SchemaToUse);
            string sqlText = string.Format("SELECT SEGMENT_CONTRAST, CHARACTER_WIDTH, CHARACTER_HEIGHT FROM " + Defaults.DB_Schema + ".XX_VDE_ITEM_PARAMS WHERE STATION_ID = {0} AND LABEL_ID = {1} AND VDE_TYPE = 0", stationid, label_id);
            OracleCommand oCmd = new OracleCommand(sqlText, conn);
            try
            {
                if (conn.State == ConnectionState.Open)
        ...
        public static bool SaveInspectionParams(InspectionParams iparams)
        {
            ...
            try
            {
                ...
                if (conn == null)
                    OracleDatabase.OpenOracleConnection(Defaults.SchemaToUse);
    ...
    public static class OracleDatabase
    {
        ...
        public static List<LabelItemDataSetup> LabelDataItems(string lin, string reel_lpn)
        {
            List<LabelItemDataSetup> retVal = new List<LabelItemDataSetup>();
            OracleCommand oCmd = new OracleCommand();
            OracleDataAdapter da = new OracleDataAdapter(oCmd);
            string sSQL = "";
            try
            {
                ...
                sSQL = string.Format("SELECT distinct(VARIABLE_NAME), PLACE_HOLDER, FIELD_TYPE FROM " + Defaults.DB_Schema + ".XX_APP_LABEL_ITEM_VAR_NAMES_VW WHERE LABEL_ITEM = '{0}'", lin);
                oCmd = new OracleCommand(sSQL, m_Conn);
                oCmd.CommandType = CommandType.Text;
                DataSet ds = new DataSet();
                da = new OracleDataAdapter();
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        LabelItemDataSetup lid = new LabelItemDataSetup();
                        lid.LIN = lin;
                        //lid.Version = version;
                        ...
                        lid.Repeat = GetRepeat(lid.VARIABLE_NAME, reel_lpn);
                        lid.VDE_DATA_LIST = GetAllMedData(lid.VARIABLE_NAME, reel_lpn);
                        lid.VDE_SEQUENCE_LIST = GetAllMedSequenceData(reel_lpn);
                        if (lid.DataPresent)
                        {
                            lid.VDE_DATA = GetFirstMedData(lid.VARIABLE_NAME, reel_lpn);
                            retVal.Add(lid);
        ...
        public static MedData LabelDataInspection(string lwo, string reel_lpn, string placeholder)
        {
            ...
            try
            {
                ...
                //sSQL = string.Format("SELECT MED_ID, TO_NUMBER(MED_SEQUENCE_NUMBER) AS medSeqNum FROM " + Defaults.DB_Schema + " .XX_VARIABLE_DATA_EXTRACT_VW WHERE LABEL_WORK_ORDER ='{0}' ORDER BY medSeqNum DESC", lwo);
                //oCmd = new OracleCommand(sSQL, m_Conn);
                //oCmd.CommandType = CommandType.Text;
                //DataSet ds = new DataSet();
                //da = new OracleDataAdapter();
                ...
                //    string sample = dr["MED_ID"].ToString();
                //    string sampleSEQ = dr["MED_SEQUENCE_NUMBER"].ToString();
                //}

                sSQL = string.Format("SELECT {0}, MED_ID, TO_NUMBER(MED_SEQUENCE_NUMBER) AS medSeqNum FROM " + Defaults.DB_Schema + ".XX_VARIABLE_DATA_EXTRACT_VW WHERE REEL_LPN = '{1}' ORDER BY medSeqNum DESC", placeholder, reel_lpn);
                oCmd = new OracleCommand(sSQL, m_Conn);
                oCmd.CommandType = CommandType.Text;
                DataSet ds = new DataSet();
                da = new OracleDataAdapter();
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    DataRow drInitRow = ds.Tables[0].Rows[0];
                    string data = drInitRow[0].ToString();
                    string seq = drInitRow["medSeqNum"].ToString();
                    MedData md = new MedData(data, seq);
                    md.PlaceHolder = placeholder;
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        md.data_list.Add(dr[0].ToString());
                        md.sequence_list.Add(dr["medSeqNum"].ToString());
                    }
                    retVal = md;
                }
            }
            ...
            {
                retVal = null;
                ErrorDesription = string.Format("LabelDataInspection() err: {0}\nSQL: {1}", ex.Message, sSQL);
                notifyError(ErrorDesription, "Data Access", true);
            }
            return retVal;
        }
        ...
        public static string GetFirstMedData(string varname, string reel_lpn)
        {
            OracleCommand oCmd = new OracleCommand();
            OracleDataAdapter da = new OracleDataAdapter(oCmd);
            string sSQL = "";
            ...
            try
            {
                ...
                sSQL = string.Format("SELECT DISTINCT({0}), TO_NUMBER(MED_SEQUENCE_NUMBER) AS medSeqNum FROM " + Defaults.DB_Schema + ".XX_VARIABLE_DATA_EXTRACT_VW WHERE REEL_LPN = '{1}' ORDER BY medSeqNum DESC", varname, reel_lpn);
                ...
                {
                    DataRow dr = ds.Tables[0].Rows[0];
                    retVal = dr[varname].ToString();
                }
                else
                    throw new Exception(string.Format("Unable to extract {0} VDE data for setup. ", varname));
            }
            catch (Exception ex)
            {
                retVal = null;
        ...
        public static List<string> GetAllMedData(string varname, string reel_lpn)
        {
            List<string> retVal = new List<string>();
            OracleCommand oCmd = new OracleCommand();
            OracleDataAdapter da = new OracleDataAdapter(oCmd);
            ...
            try
            {
                ...
                sSQL = string.Format("SELECT {0}, TO_NUMBER(MED_SEQUENCE_NUMBER) AS medSeqNum FROM " + Defaults.DB_Schema + ".XX_VARIABLE_DATA_EXTRACT_VW WHERE REEL_LPN = '{1}' ORDER BY medSeqNum DESC", varname, reel_lpn);
                ...
                if (ds.Tables[0].Rows.Count != 0)
                {
                    for(int x = 0; x < y; x++)
                        ...
                        retVal.Add(dr[varname].ToString());
                    }
                }
                else
                    throw new Exception(string.Format("Unable to extract {0} VDE data for setup. ", varname));
        ...
        public static List<string> GetAllMedSequenceData(string reel_lpn)
        {
            List<string> retVal = new List<string>();
            OracleCommand oCmd = new OracleCommand();
            OracleDataAdapter da = new OracleDataAdapter(oCmd);
            ...
            try
            {
                ...
                sSQL = string.Format("SELECT TO_NUMBER(MED_SEQUENCE_NUMBER) AS medSeqNum FROM " + Defaults.DB_Schema + ".XX_VARIABLE_DATA_EXTRACT_VW WHERE REEL_LPN = '{0}' ORDER BY medSeqNum DESC", reel_lpn);
