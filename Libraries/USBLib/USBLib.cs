    ...
    public class USBLib
    {
        ...
        private class Win32Wrapper
        {
            ...
            //DEV_BROADCAST_OEM Structure
                /*typedef struct _DEV_BROADCAST_OEM {
                  LPNRD dbco_size;
                  LPNRD dbco_devicetype;
                  LPNRD dbco_reserved;
                  LPNRD dbco_identifier;
                  LPNRD dbco_suppfunc;
            ...
            [DllImport("setupapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
            public static extern bool SetupDiGetDeviceRegistryProperty(IntPtr DeviceInfoSet, ref SP_DEVINFO_DATA DeviceInfoData, UInt32 Property, ref UInt32 PropertyRegDataType, IntPtr PropertyBuffer,
            ...
            public static extern IntPtr SetupDiOpenDevRegKey(IntPtr hDeviceInfoSet, ref SP_DEVINFO_DATA DeviceInfoData, UInt32 Scope, UInt32 HwProfile, UInt32 KeyType, UInt32 samDesired);
            ...
            public static extern int RegQueryValueEx(IntPtr hKey, string lpValueName, UInt32 lpReserved, out UInt32 lpType, System.Text.StringBuilder lpData, ref UInt32 lpcbData);
        ...
        public static bool GetUSBDevice(UInt32 VID, UInt32 PID, ref List<DeviceProperties> ListOfDP, bool GetCOMPort, Nullable<UInt32> MI=null)
        {
            ...
            try
            {
                ...
                Status = (ListOfDP.Count > 0);
            }
            catch (Exception ex)
            {
                string err = "GetUSBDevice() err: " + ex.Message;
                SystemMessageEventArgs smea = new SystemMessageEventArgs(err, "Create LPN", (int)Enums.CriticalLevels.Black);
                SMH?.Invoke(smea);
            }
            finally
            {
                try { Win32Wrapper.SetupDiDestroyDeviceInfoList(h); } catch { } //Clean up the old structure we no longer need. } 
                try { Marshal.FreeHGlobal(IntPtrBuffer); } catch { }
            }
            return Status;
        }
